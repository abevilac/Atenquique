...
\title{\textbf{Probabilistic forecasts based on plausible flows from Nevado de Colima (MX) using  data from Atenquique debris flow, 1955}}
...
\begin{enumerate}
\item For each model $M_j\in\mathcal M$, we initially set up a uniformly probabilized \emph{general input space} that we can define as arbitrary wide, namely $(\Omega^j_0,\mathcal F_0^j, P^j_0)$. At this stage we only require essential properties of feasibility, namely the existence of the numerical output and the realism of the underlying physics.
\item After a preliminary screening, we become able to characterize a \emph{specialized input space} $\Omega^j\subseteq\Omega^j_0$, under additional requirements of plausibility which are related to the macroscopic properties of the outputs. For instance, a robust numerical simulation without spurious effects, meaningful flow dynamics, and/or the capability to inundate a designated region.
...
\end{enumerate}
All these subspaces, if not negligible, are trivially probabilized by the push-forward measures $P^j$ and $P^j_i$ of their inclusion in $\Omega^j_0$, up to the multiplicative constant $P^j_0(\Omega^j_i)$, $\forall i,j$. Figure \ref{scheme} shows an example scheme of our approach.
...
\end{itemize}
We remark that the implementation of multiple models is a fundamental aspect in our approach. Indeed, the global set of feasible inputs $\Omega_G:=\bigcup_j \Omega^j_0$, is the domain on which we define our approach (Fig.\ref{scheme}). Considering multiple models can considerably extend the size of $\Omega_G$. Moreover, we show that the solution of the partial inverse problems and the model selection are strongly related.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Scheme_ok2.png}
\caption{Diagram of input spaces and partial solutions subsets. Black dots represent input values}
\label{scheme}
\end{figure}
...
The input space boundaries $\left(a_{k,{M_j}},b_{k,{M_j}}\right)_{1\le k\le d_j}$ of $\Omega_0^j$ are constrained by general assumptions:
\begin{itemize}
...
In particular, we select five sites and we gather detailed results from our simulations in those spatial locations. These are called Site \#1-\#5 and are reported as stars in Figure 1. They all belong to the set of the sixty sections studied in \cite{Saucedo2008}, and the corresponding section numbers are also reported. In detail they are:
\begin{itemize}
...
These are not described in detail, but the related results are included in Supporting Information SI4-SI5. Vice versa, we focus our analysis on the other three points, all placed along the main ravine in proximity of Atenquique village.
\begin{itemize}
...
First of all, we report the spatial maps of maximum of flow height $h$ and of kinetic energy $\kappa$:
\begin{equation}
...
Figure \ref{Fig3} reports the maximum flow height $H$, and Figure \ref{Fig4} the maximum kinetic energy $K$. MC shows the lowest values of both flow height and energy, while VS the highest, especially in the distal part of the domain. In MC, the flow in the tributaries is not capable to reach the village, while in the 95$^{th}$ percentile maps of PF and VS, it is. In VS the flow in Arroyo Pl\'atanos joins the main ravine also in the 5$^{th}$ percentile map. In general, local maxima of flow height are located in the ravine, while the kinetic energy shows a more regular decrease. The energy values on the head of the flow are in $[1,\ 10]\  m^3/s^2$, meaning a relatively slow flow compared to the dynamics observed upstream. Significant over-spill issues are absent, but in the 95$^{th}$ percentile maps all the models report some flow in the Dos Volcanes ravine, of about $[1,\ 5]\ m$ maximum height, slightly less in PF.
...
\label{Fig6}
\end{figure}
Figure \ref{Fig5} shows the local properties in site \#3, located $\sim$2 km upstream from Atenquique village. In MC, the flow reaches the site in $[600,\ 900]$ s, with a flow height of initially $[5,\ 7]$ m, and $[3,\ 5]$ m at $t_f=2400$ s. Flow speed is initially in $[1.5,\ 4.5]\ m/s$, and then one third of these values at $t_f$. Only basal friction can be dominant, with gravitational force 40\% of it, and internal friction 25\%, on average. In PF the flow reaches the site at $[300,\ 1000]$ s, with a flow height of initially $[4,\ 8]$ m with a short lasting peak of $10$ m in the 95$^{th}$ percentile, and $[3,\ 5]$ m at $t_f$. Flow speed is initially in $[4,\ 6]\ m/s$, and has a short peak of almost $14\ m/s$ in the 95$^{th}$ percentile. It becomes $[2.0,\ 2.5]\ m/s$ at $t_f$. Either basal friction, gravity, or pressure force (related to the gradient of thickness) can be dominant. Gravitational force gives 80\% expected contribution, while basal friction 70\% and pressure force 45\%. In VS, the flow first reaches the site in $[300,\ 800]$ s, with a flow height of $[1.5,\ 12]$ m initially, with a short peak at $14$ m, and becoming $[1.5,\ 5]$ at $t_f$. Flow speed is in $[1,\ 15]\ m/s$ initially, has a short peak at $20\ m/s$, and then becomes $[1.5,\ 3.5]\ m/s$ at $t_f$. Either basal friction or gravity can be dominant, both with 75\% expected contribution. We note that gravity tends to slightly decrease its expected contribution in the equations, as well as its dominance factor, through time. This is a numerical effect related to the variation of the mesh, which changes our approximation of local slope.
\begin{figure}[H]
\centering
...
Figure \ref{Fig6} shows the local properties in site \#4, located immediately before Atenquique village. In MC, the flow reaches the site in $[1300,\ 2100]$ s, with a flow height of $[2,\ 5.5]$ m. Flow speed is $<0.6\ m/s$, and $0.1\ m/s$ on average. Only basal friction can be dominant, with gravitational force 30\% of it, and internal friction 10\%, on average. In PF the flow reaches the site at $[600,\ 1800]$ s, with a flow height of $[3,\ 6]$ m, stable in time. Flow speed is initially in $[1,\ 4]\ m/s$ and half of these values at $t_f$. Either basal friction or gravity can be dominant. Gravitational force can give 95\% expected contribution, while basal friction 65\% and pressure force 25\%. In VS, the flow reaches the site in $[250,\ 550]$ s, with a first wave of $<3$ m, and then a second wave $[1.5,\ 8]$ m arrives, and stays stable. Flow speed also shows two waves - it is $<19\ m/s$ initially, and then $<12\ m/s$, with a gap in the middle. It is $[1,\ 3]\ m/s$ at $t_f$. Either gravity or basal friction can be dominant, with 95\% and 80\% expected contribution respectively. In the initial fast wave also the speed dependent friction can be dominant with 25\% chance.

In summary, all the models show lower flow height and speed than in the previous site. The flow height is stable without decreasing, meaning the formation of a significant deposit. MC is much slower than the other models, and its dynamics is completely dominated by basal friction. A very evident initial short lasting wave of hight speed is observed in VS. This fast wave is related to source \#5 in Arroyo Plat\'anos, which can anticipate the rest of the flow.

Figure \ref{Fig7} shows the local properties in site \#5, $\sim$1 km downstream inside Atenquique village. In MC, the flow only starts to inundate the site after $2100$ s, with a flow height $<3$ m. Only 25\% of the input space reaches the site. Flow speed is $<0.5\  m/s$, and ten times lower on average. Only basal friction can be dominant. In PF the flow reaches the site at $[900,\ 2400]$ s, with a flow height $<5$ m. Flow speed is initially $<3\ m/s$, and less than half of this value at $t_f$. Basal friction dominates the dynamics, with 45\% expected contribution of gravitational force and 35\% of pressure force. In VS, the flow reaches the site in $[300,\ 600]$ s, with a first wave of $<2$ m, and then a second wave $<8$ m. This second wave becomes $[2,\ 5.5]$ m at $t_f$. Flow speed is $<8\ m/s$ in the first wave, and then $<7\ m/s$ in the second, with a gap in the middle. Its average is stable at $1.5\ m/s$. Either basal friction or gravity can be dominant, with 95\% and 70\% expected contribution respectively.

In summary, the models further decrease flow height and speed compared to the previous site. In MC the site is not always reached, and in PF some input values inundate the site only at the end of the time domain. In PF, the restriction of the inputs over lower dimension subspace produces a lag of about $100$ s, possibly motivated by $\phi_2=15^\circ$ being higher than the middle of its range. The fast wave in VS is again related to source \#5.

\section{The likelihood of uncertain data}\label{s4}
Figures \ref{Fig8} and \ref{Fig9} show the flow height and speed histograms at the three selected sites, either of their maximum and of their value at $t=2400$ s. These confirm what summarized in the previous section. Probability density values reported are evaluated with respect to m and m/s, respectively. In Figure \ref{Fig8}:
\begin{itemize}
\item\textbf{Site \#3} Flow height pdf at $t_f$ is above $0.1$ over: $[3,\ 5.5]$ m in MC, $[2,\ 4.5]$ m in PF and $[1.5,\ 4.5]$ m in VS. The first two models are more peaked, while the third produces a tail of very large thickness values, up to $8$ m. Maximum height pdf is above $0.1$ over: $[5.5,\ 8]$ m in MC, $[6,\ 9]$ m in PF, $[8,\ 13]$ in VS.
\item\textbf{Site \#4} Flow height pdf at $t_f$ is above $0.1$ over: $[2.5,\ 6]$ m in MC, $[3,\ 6]$ m in PF and $[2,\ 7]$ m in VS. Maximum height pdf is above $0.1$ over: $[2.25,\ 6]$ m in MC, $[5,\ 7]$ m in PF, $[6,\ 9.5]$ m in VS. The three models show well separate modal values at $4$, $6$ and $8$ m respectively.
\item\textbf{Site \#5} In MC and PF do not always reach the site in $T=[0,\ 2400]$ s, so they both show a modal value of flow height in $0$ m. The flow height pdf at $t_f$ is above $0.1$ over: $[1,\ 1.5]$ m in MC with a tail up to $3.5$ m, $[3,\ 5.5]$ m in PF, $[2,\ 6]$ m in VS. Maximum height pdf is almost equal to final height in MC, and it is above $0.1$ over: $[3.5,\ 6]$ m in PF, $[3.5,\ 9]$ m in VS. Both PF and VS show a modal value at $5$ m.
\end{itemize}
\begin{figure}[H]
\centering
...
In Figure \ref{Fig9}:
\begin{itemize}
\item\textbf{Site \#3} Flow speed pdf at $t_f$ is above $0.25$ over: $[0.1,\ 1.4]$ m/s in MC, $[1.6,\ 2.4]$ m/s in PF and $[1.4,\ 2.8]$  m/s in VS. MC is bimodal at $0.5$ m/s and $1$ m/s, while PF has a modal value at $2.1$ m/s. Maximum speed pdf is above $0.05$ over: $[1,\ 7]$ m/s in MC, $[4,\ 8]$ m in PF, $[4,\ 16]$ in VS. PF and VS have tails up to $25$--$30$ m/s.
...
\item\textbf{Site \#5} Flow speed pdf at $t_f$ is above $0.25$ over: $[0.0,\ 0.4]$ m/s in MC, $[0.0,\ 1.4]$ m/s in PF and over $[0.0,\ 0.2]$ and $[0.4,\ 2.2]$  m/s in VS. Maximum speed pdf is above $0.05$ over: $[0.0,\ 0.2]$ and $[0.4,\ 1.2]$ m/s in MC, to $[0,\ 4]$ m in PF, $[0,\ 9]$ in VS. PF and VS have tails up to $8$ and $13$ m/s respectively.
\end{itemize}

\subsection{Alternative performance scores of the models}
In the Figures \ref{Fig8} and \ref{Fig9} we also display the empirical data concerning the observed quantities. These intervals are examples of uncertain data to test our methodology. Further research could refine/modify them.
\begin{enumerate}
...
\item the flow height in Atenquique village, from historical documents and witnesses, \\$[8,\ 9]$ m in \textbf{Site \#3} and/or \textbf{Site \#4} \citep{PonceSegura1983, Saucedo2008}.
\item the flow speed after the inundation of the village, from a comparison to analog flows, \\$[4,\ 6]\ m/s$ in \textbf{Site \#4} and/or \textbf{Site \#5} \citep{Pierson1985, Saucedo2008}.
\end{enumerate}
\begin{figure}[H]
\centering
...
Example \#1 focuses on the deposit thickness data in Sites \#3, \#4, \#5. Instead, Example \#2 and \#3 consider the deposit thickness only in Site \#5. Additionally, Example \#2 evaluates the maximum flow height in Site \#4 and the maximum flow speed in Site \#5, while Example \#3 does that in Site \#3 and Site \#4, respectively.
...
Figure \ref{Fig11} concerns Example \#2. In MC and PF the required maximum flow height is never produced. In the first model, also the required maximum flow speed is never achieved. Thus, $\tilde\Theta_2^j\neq\emptyset$ only if $j=\textrm{VS}$. The partial solution inputs in $\tilde\Theta_2^{\textrm{VS}}$ are bounded by:
...
In the spatial maps, PF shows slightly lower max flow height and significantly lower energy than VS, especially in the distal part of the domain. The flow in the tributaries is capable to reach the village, except for the smallest Arroyo Pl\'atanos in PF, which however at $t_f$ is only tens of meters from the main branch. Like in the unconditional maps, local maxima of flow height are located in the ravine, while the kinetic energy shows a more regular decrease.
...
\section{Conclusions}
In this study we have introduced a new prediction-oriented method for volcanic hazard assessment based on multiple geophysical mass flow models. In particular, our approach has the advantage to decompose the original inverse problem into a hierarchy of simpler problems, and allows the exploration of the impact of synthetic flows that are similar to those which occurred in the past, but differ in plausible ways.
