...
Considering multiple models can considerably improve the exploration of $D_G$. The quality of the solutions will clearly improve as we add more models based on new knowledge of the underlying processes. Moreover, we show that the solution of the partial inverse problems and the model selection are strongly related.
...
\end{itemize}
Our analysis concerned the mean values and also the uncertainty percentiles of the quantities of interest. Moreover, the probabilistic setting allowed us to implement the uncertainty affecting observed data. We also included information about contributing variables, which shed light over the different assumptions underlying the models. In particular, MC is generally characterized by a lower speed over its feasible input space, if compared to the other models. The expected contribution of the internal friction is not negligible, meaning the internal shear of the material is important. In PF, the pressure force contribution related to the steeping of the front front can be significant, and it can even be the dominant force. An initial short lasting wave of high speed related to the closest of the multiple sources was observed in either PF or VS. The uncertainty affecting height and speed is generally higher in VS than in PF, in spite of the higher dimensionality of the second.
