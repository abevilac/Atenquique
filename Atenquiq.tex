...
In a probabilistic framework, for each model $M\in\mathcal M$ we define $\left(M, P_{M}\right)$, where $P_M$ is a probability measure over the parameter space of $M$. While the support of $P_M$ can be restricted to a single value by solving an inverse problem for the optimal reconstruction of a particular flow, the inverse problem is not always well-posed \citep{Tarantola1982,  Tarantola1987}. That is, no input data, or multiple input data, are able to produce outputs consistent with the observed information. {\bf Sometimes the strict replication of a past flow may lead to overconstraining the model, especially if we are interested in the general predictive capabilities of a model over a whole range of possible future events.} In this study, we  generalize a poorly constrained inverse problem, decomposing it into a hierarchy of simpler problems. Our purpose is to provide a new prediction-oriented formulation for use in hazard assessment problems.
...
\end{enumerate}
We note that, $\forall j$, all the described subspaces of $\Omega^j_0$, if not negligible, are trivially probabilized by the measures $P^j$ and $P^j_i$, defined by:
$$P^j(A)=P_0^j(A)/P^j_0(\Omega^j_i),$$
for each measurable set $A\in\mathcal F^j:=\left\{\Omega^j_i \cap A:\ A\in\mathcal F_0^j\right\}$.
...
Considering multiple models can considerably improve the exploration of $D_G$. The quality of the solutions will clearly improve as we add more models based on new
knowledge of the underlying processes. Moreover, we show that the solution of the partial inverse problems and the model selection are strongly related.
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Scheme_ok3.png}
\caption{Diagram of input spaces, model functions, and output space, with feasible inputs domain (red), plausible output codomain (green), and observed data and partial solutions subsets (orange).}
\label{scheme}
\end{figure}
...
\begin{itemize}
\item[(i)] the flow reaches a minimum elevation $z<1200$ m a.s.l. before $t=1200$ s,
...
Figure \ref{scheme2} summarizes all the steps of our approach. We remark that the comparison of observed data to the statistical summary of the outputs can provide some information on the possibly unexplored portions of $D_G$, and on the resulting necessity of implementing additional models.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{diagram.pdf}
\caption{Diagram of the steps of our meta-modeling approach. Our statistical summary will include the local analysis if contributing variables.}
\label{scheme2}
\end{figure}
...
\end{itemize}
We note that close to Site \#4 there are the supports of the new bridge on the freeway to the city of Colima.

\subsection{Percentile maps of maximum flow depth and kinetic energy}
First of all, we report the spatial maps of maximum flow depth, $h$, and  kinetic energy ,$\kappa$ \footnote{While the use of dynamic pressure is more common in these applications we have used kinetic energy as a more stable
indicator of potential to damage infrastructure and a quantity whose computation as a conserved scalar across all models is more stable.}:
\begin{equation}
...
\begin{itemize}
  \item the deposit thickness is the three selected sites is not reproduced by any of the models. In particular, the input choices that fit the site upstream are inconsistent with those that fit the deposit downstream. This points to the necessity of testing different models, for example including an erosion term in the mass conservation equation.
...
In conclusion, models using MC based rheologies are unlikely to   reproduce the properties of the 1955 flow.  Instead, the flexible basal friction angle in PF allows for both higher speed and longer runout, consistent with those observed. The higher dimensionality of the parameter space is not increasing the uncertainty affecting the outputs of PF, compared to the other models. Finally, the velocity dependent term in VS is a very robust mechanism for preserving numerical stability, avoiding the spurious results that affect the MC model at equivalently low values of basal friction. Indeed the most high levels of simulated speed are observed with VS. Additional information on the speed properties in the village could allow us to further discriminate the performance of the models.
...
In particular, MC is generally characterized by a lower speed over its feasible input space, if compared to the other models. The expected contribution of the internal friction is not negligible, meaning the internal shear of the material is also important. In PF, the pressure force contribution can be significant, and it can even be the dominant force. This is related to the steepening of the flow front. An initial short lasting wave of high speed can be observed in either PF or VS. The uncertainty affecting height and speed is generally higher in VS than in PF, in spite of the higher dimensionality of the second.
...
\end{itemize}
In particular, when attempting to correctly represent the deposits, MC performs well about 2 km upstream from the village, while VS does so in the village. In the evaluation of the maximum flow depth in the village, both PF and VS can replicate those values 2 km upstream, but only VS can replicate the same values in the village. If we focus on the maximum flow speed, both PF and VS perform moderately well in the village, but only VS can provide the same speed values also 1 km downstream.
