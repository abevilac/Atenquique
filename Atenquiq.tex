...
Figures \ref{Fig10} and \ref{Fig11} show the histograms of flow height and speed at the three selected sites, either their maximum and their value at $t=2400$ s. They confirm what summarized in the previous section. In particular, in Figure \ref{Fig10}:
\begin{itemize}
\item\textbf{Site \#3} Flow height pdf at $t_f$ is above $\frac{10\%}{m}$ over: $[3,\ 5.5]$ m in MC, $[2,\ 4.5]$ m in PF and $[1.5,\ 4.5]$ m in VS. The first two models are more peaked, while the third produces a tail of very large thickness values, up to $8$ m. Maximum height pdf is above $\frac{10\%}{m}$ over: $[5.5,\ 8]$ m in MC, $[6,\ 9]$ m in PF, $[8,\ 13]$ in VS.
\item\textbf{Site \#4} Flow height pdf at $t_f$ is above $\frac{10\%}{m}$ over: $[2.5,\ 6]$ m in MC, $[3,\ 6]$ m in PF and $[2,\ 7]$ m in VS. Maximum height pdf is above $\frac{10\%}{m}$ over: $[2.25,\ 6]$ m in MC, $[5,\ 7]$ m in PF, $[6,\ 9.5]$ m in VS. The three models show well separate modal values at $4$, $6$ and $8$ m respectively.
\item\textbf{Site \#5} In MC and PF do not always reach the site in $T=[0,2400]$ s, so they both show a modal value of flow height in $0$ m. The flow height pdf at $t_f$ is above $\frac{10\%}{m}$ over: $[1,\ 1.5]$ m in MC with a tail up to $3.5$ m, $[3,\ 5.5]$ m in PF, $[2,\ 6]$ m in VS. Maximum height pdf is almost equal to final height in MC, and it is above $\frac{10\%}{m}$ over: $[3.5,\ 6]$ m in PF, $[3.5,\ 9]$ m in VS. Both PF and VS show a modal value at $5$ m.
\end{itemize}

In Figure \ref{Fig11}:
\begin{itemize}
\item\textbf{Site \#3} Flow speed pdf at $t_f$ is above $\frac{25\%}{m/s}$ over: $[0.1,\ 1.4]$ m/s in MC, $[1.6,\ 2.4]$ m/s in PF and $[1.4,\ 2.8]$  m/s in VS. MC is bimodal at $0.5$ m/s and $1$ m/s, while PF has a modal value at $2.1$ m/s. Maximum speed pdf is above $\frac{5\%}{m/s}$ over: $[1,\ 7]$ m/s in MC, $[4,\ 8]$ m in PF, $[4,\ 16]$ in VS. PF and VS have tails up to $25-30$ m/s.
\item\textbf{Site \#4} Flow speed pdf at $t_f$ is above $\frac{25\%}{m/s}$ over: $[0.00,\ 0.45]$ m/s in MC, $[0.6,\ 1.8]$ m/s in PF and $[0.5,\ 3.0]$  m/s in VS. PF has a modal value at $1$ m/s. Maximum speed pdf in MC is concentrated below $1$ m/s. In PF it is above $\frac{25\%}{m/s}$ over $[1,\ 5]$ m/s. In VS it is bimodal and above $\frac{25\%}{m/s}$ over: $[2,\ 6]$ m/s and $[12, 20]$ m/s.

\item\textbf{Site \#5} Flow speed pdf at $t_f$ is above $\frac{25\%}{m/s}$ over: $[0.0,\ 0.4]$ m/s in MC, $[0.0,\ 1.4]$ m/s in PF and over $[0.0,\ 0.2]$ and $[0.4,\ 2.2]$  m/s in VS. Maximum speed pdf is above $\frac{5\%}{m/s}$ over: $[0.0,\ 0.2]$ and $[0.4,\ 1.2]$ m/s in MC, to $[0,\ 4]$ m in PF, $[0,\ 9]$ in VS. PF and VS have tails up to $8$ and $13$ m/s respectively.
\end{itemize}

\subsection{Alternative performance scores of the models}
In the Figures \ref{Fig10} and \ref{Fig11} we display the empirical information concerning these quantities. In particular:
\begin{enumerate}
...
\caption{Example \#2 of partial solutions inputs in (a-b) MC, (c-d) PF, (e-f) VS experimental design. (a-c-e) are projected along the $V$ coordinate, and (b-d-f) along $\phi_{int}$, $\phi_2$ and $\xi$ coordinates, respectively. The color expresses the considered data: yellow is deposit thickness in Site \#5 \citep{Saucedo2008}, blue is wave height in Site \#4 \citep{PonceSegura1983}, red is flow speed in Site \#5 \citep{Pierson1985}.}
\label{Fig11}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{Fig11_PF.png}
\caption{Example \#3 of partial solutions inputs in (a-b) MC, (c-d) PF, (e-f) VS experimental design. (a-c-e) are projected along the $V$ coordinate, and (b-d-f) along $\phi_{int}$, $\phi_2$ and $\xi$ coordinates, respectively. The color expresses the considered data: yellow is deposit thickness in Site \#5 \citep{Saucedo2008}, blue is wave height in Site \#3 \citep{PonceSegura1983}, red is flow speed in Site \#4 \citep{Pierson1985}.}
\label{Fig11a}
\end{figure}
...
\caption{Partial solutions inputs in PF experimental design supported over one of the four different hyperplanes described in Table 1, (a-c) data refer to Figure \ref{Fig11_0}, (c-d) to Figure \ref{Fig11}, (e-f) to Figure \ref{Fig11a}. All plots are projected along the $V$ coordinate. The color expresses the considered data: yellow is deposit thickness \citep{Saucedo2008}, red is flow speed \citep{Pierson1985}.}
\label{FigA2}
\end{figure}
